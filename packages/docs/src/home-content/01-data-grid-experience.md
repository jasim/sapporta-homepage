...
80% of most ERP software is just forms, data grids, and reports, and all of
them easy to operate using the keyboard. Sapporta tries to bring the same
experience to database applications on the web. It helps you build
applications that operate mostly on relational data, like personal databases,
operational tools, and business software.
...
Data grids are pervasive in Sapporta. They are used in regular tables, inside
forms, and for reports. The value of a data grid is that it is a general-purpose
surface. You can put a lot of heart and soul into a single component, and it
benefits the entire application.
...
and together they form a complete algebra for working with tabular data.

Building these interactions in a conventional web application, even with an LLM,
...
creates reports with summary rows, detail rows, and drill-down links. The detail
rows use the same data grid and thus have all the same affordances.
